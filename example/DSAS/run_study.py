...
def setup_run_dir(run_id: int, params: dict) -> Path:
   ...
   # shutil.copy(
     # Path("/app/dsas/Materials.xml"),
     # run_dir / "Materials.xml"
   #)
    # Copy required static files into each run directory
    shutil.copy(Path("/app/dsas/Materials.xml"),                    run_dir / "Materials.xml")
    shutil.copy(Path("/app/dsas/Surfaces.xml"),                     run_dir / "Surfaces.xml") 
    shutil.copy(Path("/app/dsas/examples1/DSAS/energy.txt"),        run_dir / "energy.txt")
    shutil.copy(Path("/app/dsas/example/DSAS/GateMaterials.db"), run_dir / "GateMaterials.db")
    shutil.copy(
      Path("/app/dsas/example/DSAS/GateMaterials.db"),
      run_dir/ "GateMaterials.db"
    )
...
def main():
    ...
    if failed:
        ...
        for r in failed:
            log_path = RUNS_DIR / f"run_{r['run_id']:03d}" / "gate.log"
            print(f"  run_{r['run_id']:03d}  ->  check {log_path}")
        sys.exit(1)
