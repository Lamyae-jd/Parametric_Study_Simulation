...
void GateRootHitBuffer::Clear()
{
  ...
  decayType = 0;
  gammaType = 0;
  phantomComptonHitPosX = new std::vector<G4double>;
  phantomComptonHitPosY = new std::vector<G4double>();
  phantomComptonHitPosZ = new std::vector<G4double>();
  phantomComptonHitTime = new std::vector<G4double>();
  phantomRayleighHitPosX = new std::vector<G4double>();
  phantomRayleighHitPosY = new std::vector<G4double>();
  phantomRayleighHitPosZ = new std::vector<G4double>();
  phantomRayleighHitTime = new std::vector<G4double>();
  strcpy (processName, " ");
...
void GateRootHitBuffer::Fill(GateCrystalHit* aHit)
{
  ...
  phantomComptonHitPosX = aHit->GetPhantomComptonHitPosX().get();
  phantomComptonHitPosY = aHit->GetPhantomComptonHitPosY().get();
  phantomComptonHitPosZ = aHit->GetPhantomComptonHitPosZ().get();
  phantomComptonHitTime = aHit->GetPhantomComptonHitTime().get();
  phantomRayleighHitPosX = aHit->GetPhantomRayleighHitPosX().get();
  phantomRayleighHitPosY = aHit->GetPhantomRayleighHitPosY().get();
  phantomRayleighHitPosZ = aHit->GetPhantomRayleighHitPosZ().get();
  phantomRayleighHitTime = aHit->GetPhantomRayleighHitTime().get();
  // HDS : septal
...
void GateHitTree::Init(GateRootHitBuffer& buffer)
{
  ...
  Branch("phantomComptonHitPosX",   (&buffer.phantomComptonHitPosX));
  Branch("phantomComptonHitPosY",   (&buffer.phantomComptonHitPosY));
  Branch("phantomComptonHitPosZ",   (&buffer.phantomComptonHitPosZ));
  Branch("phantomComptonHitTime",   (&buffer.phantomComptonHitTime));
  Branch("phantomRayleighHitPosX",  (&buffer.phantomRayleighHitPosX));
  Branch("phantomRayleighHitPosY",  (&buffer.phantomRayleighHitPosY));
  Branch("phantomRayleighHitPosZ",   (&buffer.phantomRayleighHitPosZ));
  Branch("phantomRayleighHitTime",   (&buffer.phantomRayleighHitTime));
...
void GateRootSingleBuffer::Fill(GateSingleDigi* aDigi)
{
  ...
  timeOfFlight  =  aDigi->GetTimeOfFlight()/s;
...
void GateSingleTree::Init(GateRootSingleBuffer& buffer)
{
  ...
  if ( GateSingleDigi::GetSingleASCIIMask(7) )
    Branch("time",           &buffer.time,"time/D");
  if ( GateSingleDigi::GetSingleASCIIMask(21) )
    Branch("timeOfFlight",   &buffer.timeOfFlight,"timeOfFlight/D");
