...
G4int GateSourceMgr::AddSource( std::vector<G4String> sourceVec )
{
  ...
  if( bAddSource ) {
	...
	//mSources[mSources.size()-1]->SetSourceID( m_sourceProgressiveNumber );
      ...
      else if (sourceGeomType == "pulsed") {
        source = new GateSourcePulsed( sourceName );
        source->SetType("pulsed");
        source->SetSourceID( m_sourceProgressiveNumber );
      }
      else if (sourceGeomType == "voxelPulsed") {
        source = new GateSourceVoxellizedPulsed( sourceName );
        source->SetSourceID( m_sourceProgressiveNumber );
        source->SetIfSourceVoxelized(true); 
      }
      else if (sourceGeomType == "fan") {
        source = new GateSourceFanBeam( sourceName );
        source->SetType("fan");
        source->SetSourceID( m_sourceProgressiveNumber );
      }
        ...
        GateError("Unknown source type '" << sourceGeomType
                  << "'. Known types are voxel, linacbeam, gps, pulsed, voxelPulsed and fan\n");
      }
