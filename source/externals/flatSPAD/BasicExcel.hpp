 ...
 * Created     : Tue Mar 22 11:58:16 2016 -0400 by valentin
 * 
 * FILE_INFO_END */
...
namespace RefCount
{
 ...
 // reference photon_id for SmartPtr managed objects
struct RefCnt
{
     // On construction the reference photon_id is initialized with an usage count of 0.
    RefCnt()
     :    _ref_cnt(0)
    {
    }
...
template<typename T> struct SmartPtr
{
     ...
     :    _ptr(NULL)
    {
     ...
     // The initialized SmartPtr constructor increments the reference photon_id in struct RefCnt.
    SmartPtr(T* p)
     :    _ptr(p)
    {
        if (p)
     ...
     // The copy constructor increments the reference photon_id.
     ...
     :    _ptr(other._ptr)
    {
     ...
     // The destructor decreases the reference photon_id and
     // frees the managed memory as the photon_id reaches zero.
    ~SmartPtr()
    {
        if (_ptr) {
            if (!--_ptr->_ref_cnt)
     ...
     // The assignment operator increments the reference photon_id.
