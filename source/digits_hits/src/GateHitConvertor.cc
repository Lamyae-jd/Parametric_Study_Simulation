...
void GateHitConvertor::ProcessOneHit(const GateCrystalHit* hit,GatePulseList* pulseList)
{
 /*if (hit->GetEdep()==0) {
  ...
  pulse->SetPhantomComptonHitPosX( hit->GetPhantomComptonHitPosX() );
  pulse->SetPhantomComptonHitPosY( hit->GetPhantomComptonHitPosY() );
  pulse->SetPhantomComptonHitPosZ( hit->GetPhantomComptonHitPosZ() );
  pulse->SetPhantomComptonHitTime( hit->GetPhantomComptonHitTime() );
  pulse->SetPhantomRayleighHitPosX( hit->GetPhantomRayleighHitPosX() );
  pulse->SetPhantomRayleighHitPosY( hit->GetPhantomRayleighHitPosY() );
  pulse->SetPhantomRayleighHitPosZ( hit->GetPhantomRayleighHitPosZ() );
  pulse->SetPhantomRayleighHitTime( hit->GetPhantomRayleighHitTime() );
  pulse->SetVolumeID( hit->GetVolumeID() );
  pulse->SetScannerPos( hit->GetScannerPos() );
  pulse->SetScannerRotAngle( hit->GetScannerRotAngle() );
  pulse->SetTimeOfFlight( hit->GetTrackLocalTime());
