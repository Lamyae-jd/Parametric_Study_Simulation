      ...
      if (CHC)
        {
          ...
          std::shared_ptr<std::vector<G4double>> phantomComptonHitPosXVecPhoton1 = std::make_shared<std::vector<G4double>>();
          std::shared_ptr<std::vector<G4double>> phantomComptonHitPosYVecPhoton1 = std::make_shared<std::vector<G4double>>();
          std::shared_ptr<std::vector<G4double>> phantomComptonHitPosZVecPhoton1 = std::make_shared<std::vector<G4double>>();

          std::shared_ptr<std::vector<G4double>> phantomComptonHitPosXVecPhoton2 = std::make_shared<std::vector<G4double>>();
          std::shared_ptr<std::vector<G4double>> phantomComptonHitPosYVecPhoton2 = std::make_shared<std::vector<G4double>>();
          std::shared_ptr<std::vector<G4double>> phantomComptonHitPosZVecPhoton2 = std::make_shared<std::vector<G4double>>();

          std::shared_ptr<std::vector<G4double>> phantomComptonHitTimeVecPhoton1 = std::make_shared<std::vector<G4double>>();
          std::shared_ptr<std::vector<G4double>> phantomComptonHitTimeVecPhoton2 = std::make_shared<std::vector<G4double>>();

          std::shared_ptr<std::vector<G4double>> phantomRayleighHitPosXVecPhoton1 = std::make_shared<std::vector<G4double>>();
          std::shared_ptr<std::vector<G4double>> phantomRayleighHitPosYVecPhoton1 = std::make_shared<std::vector<G4double>>();
          std::shared_ptr<std::vector<G4double>> phantomRayleighHitPosZVecPhoton1 = std::make_shared<std::vector<G4double>>();

          std::shared_ptr<std::vector<G4double>> phantomRayleighHitPosXVecPhoton2 = std::make_shared<std::vector<G4double>>();
          std::shared_ptr<std::vector<G4double>> phantomRayleighHitPosYVecPhoton2 = std::make_shared<std::vector<G4double>>();
          std::shared_ptr<std::vector<G4double>> phantomRayleighHitPosZVecPhoton2 = std::make_shared<std::vector<G4double>>();

          std::shared_ptr<std::vector<G4double>> phantomRayleighHitTimeVecPhoton1 = std::make_shared<std::vector<G4double>>();
          std::shared_ptr<std::vector<G4double>> phantomRayleighHitTimeVecPhoton2 = std::make_shared<std::vector<G4double>>();
          ...
          for (G4int iPHit=0;iPHit<NpHits;iPHit++)
            {
              ...
              G4double hitTime        = (*PHC)[iPHit]->GetTime();
              ...
              if (processName.find("ompt") != G4String::npos)
                {
                  ...
                  if (phantomTrackID == photon1ID)
                    {
                      phantomComptonHitPosXVecPhoton1->push_back(hitPos[0]);
                      phantomComptonHitPosYVecPhoton1->push_back(hitPos[1]);
                      phantomComptonHitPosZVecPhoton1->push_back(hitPos[2]);
                      phantomComptonHitTimeVecPhoton1->push_back(hitTime);
                  ...
                  if (phantomTrackID == photon2ID)
                    {
                      phantomComptonHitPosXVecPhoton2->push_back(hitPos[0]);
                      phantomComptonHitPosYVecPhoton2->push_back(hitPos[1]);
                      phantomComptonHitPosZVecPhoton2->push_back(hitPos[2]);
                      phantomComptonHitTimeVecPhoton2->push_back(hitTime);
              ...
              if (processName.find("Rayl") != G4String::npos)
                {
                  ...
                  if (phantomTrackID == photon1ID)
                    {
                      phantomRayleighHitPosXVecPhoton1->push_back(hitPos[0]);
                      phantomRayleighHitPosYVecPhoton1->push_back(hitPos[1]);
                      phantomRayleighHitPosZVecPhoton1->push_back(hitPos[2]);
                      phantomRayleighHitTimeVecPhoton1->push_back(hitTime);
                  ...
                  if (phantomTrackID == photon2ID)
                    {
                      phantomRayleighHitPosXVecPhoton2->push_back(hitPos[0]);
                      phantomRayleighHitPosYVecPhoton2->push_back(hitPos[1]);
                      phantomRayleighHitPosZVecPhoton2->push_back(hitPos[2]);
                      phantomRayleighHitTimeVecPhoton2->push_back(hitTime);
              ...
              if ((*CHC)[iHit]->GoodForAnalysis())
                {
                  ...
                  std::shared_ptr<std::vector<G4double>> phantomComptonHitPosXVec;
                  std::shared_ptr<std::vector<G4double>> phantomComptonHitPosYVec;
                  std::shared_ptr<std::vector<G4double>> phantomComptonHitPosZVec;
                  std::shared_ptr<std::vector<G4double>> phantomComptonHitTimeVec;
                  std::shared_ptr<std::vector<G4double>> phantomRayleighitPosXVec;
                  std::shared_ptr<std::vector<G4double>> phantomRayleighitPosYVec;
                  std::shared_ptr<std::vector<G4double>> phantomRayleighitPosZVec;
                  std::shared_ptr<std::vector<G4double>> phantomRayleighHitTimeVec;
                  ...
                  if (photonID == 1)
                    {
                    phantomComptonHitPosXVec = phantomComptonHitPosXVecPhoton1;
                      phantomComptonHitPosYVec = phantomComptonHitPosYVecPhoton1;
                      phantomComptonHitPosZVec = phantomComptonHitPosZVecPhoton1;
                      phantomComptonHitTimeVec = phantomComptonHitTimeVecPhoton1;
                      phantomRayleighitPosXVec = phantomRayleighHitPosXVecPhoton1;
                      phantomRayleighitPosYVec = phantomRayleighHitPosYVecPhoton1;
                      phantomRayleighitPosZVec = phantomRayleighHitPosZVecPhoton1;
                      phantomRayleighHitTimeVec = phantomRayleighHitTimeVecPhoton1;
                    ...
                    {
                    phantomComptonHitPosXVec = phantomComptonHitPosXVecPhoton2;
                      phantomComptonHitPosYVec = phantomComptonHitPosYVecPhoton2;
                      phantomComptonHitPosZVec = phantomComptonHitPosZVecPhoton2;
                      phantomComptonHitTimeVec = phantomComptonHitTimeVecPhoton2;
                      phantomRayleighitPosXVec = phantomRayleighHitPosXVecPhoton2;
                      phantomRayleighitPosYVec = phantomRayleighHitPosYVecPhoton2;
                      phantomRayleighitPosZVec = phantomRayleighHitPosZVecPhoton2;
                      phantomRayleighHitTimeVec = phantomRayleighHitTimeVecPhoton2;
                  ...
                  (*CHC)[iHit]->SetPhantomComptonHitPosX   (phantomComptonHitPosXVec);
                  (*CHC)[iHit]->SetPhantomComptonHitPosY   (phantomComptonHitPosYVec);
                  (*CHC)[iHit]->SetPhantomComptonHitPosZ   (phantomComptonHitPosZVec);
                  (*CHC)[iHit]->SetPhantomRayleighHitPosX   (phantomRayleighitPosXVec);
                  (*CHC)[iHit]->SetPhantomRayleighHitPosY   (phantomRayleighitPosYVec);
                  (*CHC)[iHit]->SetPhantomRayleighHitPosZ   (phantomRayleighitPosZVec);
                  (*CHC)[iHit]->SetPhantomComptonHitTime   (phantomComptonHitTimeVec);
                  (*CHC)[iHit]->SetPhantomRayleighHitTime   (phantomRayleighHitTimeVec);
