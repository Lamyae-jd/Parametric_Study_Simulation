...
GateToSPAD::GateToSPAD(const G4String& name, GateOutputMgr* outputMgr,GateOpticalSystem* itsSystem,DigiMode digiMode)
                    ...
                    // the VOutputModule pure virtual method GiveNameOfFile()
{
  ...
  m_isEnabled = true; // Keep this flag false: all output are disabled by default
  m_asciiMessenger = nullptr;
...
const G4String& GateToSPAD::GiveNameOfFile()
{
  return m_fileName;
}

void GateToSPAD::RecordBeginOfAcquisition()
{
...
void GateToSPAD::RecordEndOfAcquisition()
{
  if (!(m_system->GetProjectionSetMaker()->IsEnabled())) return;
...
void GateToSPAD::RecordBeginOfRun(const G4Run* )
{
  if (!(m_system->GetProjectionSetMaker()->IsEnabled())) return;
}
...
void GateToSPAD::RecordEndOfRun(const G4Run* )
{
  if (!(m_system->GetProjectionSetMaker()->IsEnabled())) return;
...
*/
void GateToSPAD::Describe(size_t indent)
{
...
void GateToSPAD::WriteGeneralInfo()
{
