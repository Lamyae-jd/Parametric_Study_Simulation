...
std::ofstream& operator<<(std::ofstream& flux, GateSingleDigi* digi)
{
  ...
  if ( GateSingleDigi::GetSingleASCIIMask(21) ) flux << " " << std::resetiosflags(std::ios::floatfield) << std::setiosflags(std::ios::scientific) << std::setw(30) << std::setprecision(23) << digi->GetTimeOfFlight()/s;
  flux << Gateendl;
